...
const excelCurrencyFormat = (currency: Option<Currency>): string => {
  ...
  if (
    isNullish(currency.format) ||
    isNullish(currency.format.symbol) ||
    isNullish(currency.format.symbol.template) ||
    isNullish(currency.format.symbol.grapheme)
  ) {
    return `#,##0.00 "${currency.currency}"`;
  }

  const symbol = currency.format.symbol.template.replace('$', currency.format.symbol.grapheme);
  return symbol.replace('1', '#,##0.00');
};

const extractInvoicesTable = ({
  invoices,
  backup,
  i18n,
  worksheet,
